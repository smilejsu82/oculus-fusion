...
public class Test3Main : MonoBehaviour, INetworkRunnerCallbacks
{
    ...
    [SerializeField] private NetworkPrefabRef grabItemPrefab;
        ...
        EventDispatcher.instance.AddListener("Spawned", (obj) =>
        {
            ...
            if (test3Player.HasInputAuthority)
            {
                ...
                { 
                    ...
                    Debug.LogFormat("hostPlayerRef: {0}", this.hostPlayerRef);

                    NetworkObject networkGrabObject = this._runner.Spawn(
                        this.grabItemPrefab,
                        new Vector3(0, 0.85f, 0.3f),
                        Quaternion.identity, hostPlayerRef);
                    
                    // networkGrabObject.GetComponent<PalmGrabItem>().Init(this._runner);
                }
                else
                {
                    //client (local)
    ...
    private PlayerRef hostPlayerRef;
    ...
    public void OnPlayerJoined(NetworkRunner runner, PlayerRef player)
    {
        ...
        if (runner.IsServer)     
        {
            if (this._spawnedCharacters.Count == 0)
            {
                //host 
                hostPlayerRef = player;
                Debug.LogFormat("hostPlayerRef: <color=yellow>{0}</color>", hostPlayerRef);
