...
public class Test3Player : NetworkBehaviour, INetworkRunnerCallbacks
{
    ...
    [Networked] public Vector3 headRotation { get; set; }
    ...
    public void Init(NetworkRunner runner)
    {
        ...
        EventDispatcher.instance.AddListener("OnGrab", data =>
        {
            var pose = (bool)data == true ? 1 : 0;
            Debug.LogFormat("pose: {0}", pose);
            this.leftHandAnim.SetInteger("Pose", pose);
        });
    }
    ...
    [Networked(OnChanged = nameof(NetworkGrabChanged))]
    public bool IsGrab { get; set; }

    private static void NetworkGrabChanged(Changed<Test3Player> changed)
    {
        Debug.LogFormat("changed.Behaviour.IsGrab: {0}", changed.Behaviour.IsGrab);
        
        EventDispatcher.instance.Dispatch("OnGrab", changed.Behaviour.IsGrab);
    }
    ...
    private void Update()
    {
        if (Object.HasInputAuthority && this.Runner.IsServer)   //host 
        {
            ...
            if (rHandTrigger > 0)
            {
                ...
                {
                    IsGrab = true;
                    Debug.LogFormat("isGrab: {0}", IsGrab);
                }
            }else if (rHandTrigger == 0)
            {
                if (IsGrab)
                {
                    IsGrab = false;
    ...
    public void OnInput(NetworkRunner runner, NetworkInput input)
    {
        ...
        data.headPosition = this.centerEyeAnchor.rotation.eulerAngles;
    ...
    public override void FixedUpdateNetwork()
    {
        if (GetInput(out NetworkInputData data))
        {
            ...
            this.headRotation = data.headRotation;
    ...
    public override void Render()
    {
        ...
        this.headTrans.rotation = Quaternion.Euler(this.headRotation);
